...
def load_imdb(tokenizer, max_length, training_batch_size, eval_batch_size, seed=42, include_sals=False):
    ...
    if include_sals:
        ...
        BEST_EPOCHS = 2#get_best_epochs(f"./directory/bert/{seed}/imdb/logs/ft_logs.json")[-3:] + 1
...
def load_rte(**kwargs):
    return load_glue_task("rte", **kwargs)

def load_mnli(**kwargs):
    return load_glue_task("mnli", **kwargs)

def load_qnli(**kwargs):
    return load_glue_task("qnli", **kwargs)

def load_mrpc(**kwargs):
    return load_glue_task("mrpc", **kwargs)

def load_cola(**kwargs):
    return load_glue_task("cola", **kwargs)

def load_stsb(**kwargs):
    return load_glue_task("stsb", **kwargs)

def load_qqp(**kwargs):
    return load_glue_task("qqp", **kwargs)
