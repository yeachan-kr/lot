...
SAVE_MODEL = True 
SEED = args.DATA_SEED
SALIENCIES_PATH = './directory/bert/22/hatexplain/data/sals/train/hatexplain_attention3.npy'#WORKER_SALIENCIES_PATH_to_READ(args.TASK, "bert", seed=SEED)
...
def load_model():
  ...
  if INITIAL_MODEL_PATH == "":
    best_epoch = 2#get_best_epochs(f"./directory/bert/{SEED}/{TASK}/logs/ft_logs.json")[-1] + 1
...
checkpoint = ModelCheckpoint_wlr(
    ...
    best_score=2#np.max(get_best_epochs(f"./directory/bert/{SEED}/{TASK}/logs/ft_logs.json", return_scores=True))
)
