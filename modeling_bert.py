...
from modeling_router import LoTPredictor
...
class BertLoTEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([BertLayer(config) for _ in range(config.num_hidden_layers)])
        ...
        self.skim_predictors = nn.ModuleList([LoTPredictor(config.hidden_size, 2) for _ in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            # gumbel-sigmoid
            skim_logits = self.skim_predictors[i]((hidden_states[:,1:,:] + hidden_states.mean(dim=1).unsqueeze(1).detach()))
            skim_mask = nn.functional.gumbel_softmax(skim_logits, hard=True, tau=1.0)
            skim_mask = skim_mask[:,:,1]
            
            # nan values to zeros
            skim_mask[skim_mask != skim_mask] = 0
            ...
            skim_mask = skim_mask_with_cls # (batch, seq)
            self.all_pos_masks.append(skim_mask)
            self.all_pos_logits.append(skim_logits)

            # adding context vectors
            unskimmed_mask = (1 - skim_mask).detach() * (attention_mask == 0.0).float().squeeze()
            unskimmed_mask[:, 0] = 1.0

            unskimmed_tokens = hidden_states * unskimmed_mask.unsqueeze(-1)
            unskimmed_tokens = unskimmed_tokens.sum(dim=1) / (1e-8 + unskimmed_mask.sum(dim=1).unsqueeze(1))
            unskimmed_tokens = unskimmed_tokens.unsqueeze(1)

            # add summarized token to context tokens
            hidden_states = torch.cat([hidden_states, unskimmed_tokens], dim=1)
            attention_mask = torch.cat([attention_mask, torch.ones(attention_mask.shape[0], 1, 1, 1, device=attention_mask.device)], dim=-1)
            skim_mask = torch.cat([skim_mask, torch.ones(skim_mask.shape[0], 1, device=skim_mask.device)], dim=-1)
